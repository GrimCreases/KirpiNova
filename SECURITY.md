...
- Account verification and login-password recovery emails are sent through the deployment owner's SMTP service. They contain the account email address and a single-use, expiring link; they never contain vault data, the vault passphrase, or workspace content. SMTP credentials remain server-side environment variables and are not included in the browser bundle or repository.
- Recovery requests return non-enumerating responses and are throttled by hashed email and network address. A successful login-password reset revokes existing account sessions but never changes, recovers, or bypasses the separate client-side vault passphrase.
